...
"[project]/frontend/app/result/page.tsx [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function ResultPage() {
    ...
    .streetview-frame:hover {
      ...
      `;
            ...
            // ========= 数学ユーティリティ =========
            const getDistanceKm = {
                "ResultPage.useEffect.getDistanceKm": (lat1, lon1, lat2, lon2)=>{
                    const R = 6371;
                    const toRad = {
                        "ResultPage.useEffect.getDistanceKm.toRad": (d)=>d * Math.PI / 180
                    }["ResultPage.useEffect.getDistanceKm.toRad"];
                    const dLat = toRad(lat2 - lat1);
                    const dLon = toRad(lon2 - lon1);
                    const a = Math.sin(dLat / 2) ** 2 + Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLon / 2) ** 2;
                    return 2 * R * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
                }
            }["ResultPage.useEffect.getDistanceKm"];
            const decodePolyline = {
                "ResultPage.useEffect.decodePolyline": (encoded)=>{
                    const pts = [];
                    let i = 0;
                    let lat = 0;
                    let lng = 0;
            ...
            }["ResultPage.useEffect.decodePolyline"];
            // ========= Street View 埋め込みヘルパ =========
            const buildStreetViewEmbedURL = {
                "ResultPage.useEffect.buildStreetViewEmbedURL": (lat, lng)=>{
            ...
            }["ResultPage.useEffect.buildStreetViewEmbedURL"];
            const isEmbeddableGoogleURL = {
                "ResultPage.useEffect.isEmbeddableGoogleURL": (url)=>{
                    if (!url) return false;
                    try {
                        const u = new URL(url);
            ...
            }["ResultPage.useEffect.isEmbeddableGoogleURL"];
            // ========= テーマ / ナビまわり =========
            const isDarkSiteTheme = {
                "ResultPage.useEffect.isDarkSiteTheme": ()=>document.body.classList.contains('dark')
            }["ResultPage.useEffect.isDarkSiteTheme"];
            let resultMap = null;
            const measureAndApplyNavHeight = {
                "ResultPage.useEffect.measureAndApplyNavHeight": ()=>{
                    ...
                    if (resultMap) {
                        ...
                        }["ResultPage.useEffect.measureAndApplyNavHeight"], 0);
                    }
                }
            }["ResultPage.useEffect.measureAndApplyNavHeight"];
            // ========= 下部ボタン & 画面遷移 =========
            const retry = {
                "ResultPage.useEffect.retry": ()=>{
            ...
            const goHome = {
                "ResultPage.useEffect.goHome": ()=>{
                    location.href = 'home';
                }
            }["ResultPage.useEffect.goHome"];
            const injectBottomButtons = {
                "ResultPage.useEffect.injectBottomButtons": (sidebarEl)=>{
                    if (!sidebarEl) return;
            ...
            }["ResultPage.useEffect.injectBottomButtons"];
            ...
            window.addEventListener('load', {
                "ResultPage.useEffect": ()=>setTimeout(measureAndApplyNavHeight, 0)
            }["ResultPage.useEffect"]);
            // メインの async ロジック
            ...
            ({
                "ResultPage.useEffect": async ()=>{
                    ...
                    const applyTiles = {
                        "ResultPage.useEffect.applyTiles": ()=>{
                            if (!resultMap) return;
                            ...
                            }["ResultPage.useEffect.applyTiles"], 0);
                        }
                    }["ResultPage.useEffect.applyTiles"];
                    applyTiles();
      ...
      `;
        ...
        `;
                            ...
                            const pts = rk.hotels.filter({
                                "ResultPage.useEffect.pts": (h)=>h.lat && h.lng
                            }["ResultPage.useEffect.pts"]).map({
                                "ResultPage.useEffect.pts": (h)=>[
                                        h.lat,
                                        h.lng
                                    ]
                            }["ResultPage.useEffect.pts"]);
                            if (pts.length > 0) {
...
_s(ResultPage, "OD7bBpZva5O2jO+Puf00hKivP7c=");
_c = ResultPage;
var _c;
...
//# sourceMappingURL=frontend_76a7695d._.js.map